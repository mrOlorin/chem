...
export async function downloadFrames(canvasDataUrlList: Array<string>) {
    ...
    for (let i = 0, length = canvasDataUrlList.length - 1; i < length; i++) {
        const n = `${i}`.padStart(4, '0');
        zip.file(`frame-${n}.png`, canvasDataUrlList[i].replace(/^data:image\/(png|jpg);base64,/, ""), {base64: true});
    }
